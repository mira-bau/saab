...
class Preprocessor:
    ...
    def fit(self, train_data: Any, schema: dict | None = None):
        ...
        # Extract sequences
        sequences = self._selected_extractor.extract(train_data, schema=schema)

        if not sequences:
            raise ValueError("No sequences extracted from training data")
    ...
    def transform(
        ...
        # Select extractor (use fitted one or auto-detect)
        if self._selected_extractor is not None:
        ...
        # Extract sequences
        sequences = extractor.extract(data, schema=schema)
